...
_clock_swap_out_victim(struct mm_struct *mm, struct Page ** ptr_page, int in_tick)
{
    ...
    while (1) {
        ...
        // 编写代码
        if(curr_ptr != head){
            list_entry_t *curr_entry = curr_ptr;
            struct Page *curr_page = le2page(curr_entry, pra_page_link);
        ...
        else curr_ptr = head->next;// 遍历页面链表pra_list_head，查找最早未被访问的页面
        // 获取当前页面对应的Page结构指针
